...
if __name__ == '__main__':
    ...
    for img_path in pics:
        ...
        print(f"class 1 pixels = {mask.sum()}, class 2 pixels = {(~mask).sum()}")
        cv.imshow("raw img", img)
        img[mask] = cv.addWeighted(img[mask], 0.7, np.zeros_like(img[mask]) + np.array([0, 0, 255]), 0.35, 0, dtype=cv.CV_8UC3)
        img[~mask] = cv.addWeighted(img[~mask], 0.7, np.zeros_like(img[~mask]) + np.array([0, 255, 0]), 0.35, 0, dtype=cv.CV_8UC3)
